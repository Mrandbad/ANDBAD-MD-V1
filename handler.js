...
chat.autoresponder = false
            ...
            if (settings) {
               ...
               if (!('restrict' in settings)) settings.restrict = false
                if (!('pair' in settings)) settings.pair = true
        ...
        if (opts['nyimak']) return // If 'nyimak' option is enabled, do nothing
        if (!m.fromMe && opts['self']) return // If 'self' option is enabled and the message is not from the bot itself, do nothing
        if (opts['swonly'] && m.chat !== 'status@broadcast') return // If 'swonly' option is enabled and the chat is not a status broadcast, do nothing
        if (typeof m.text !== 'string') // If the message text is not a string
            m.text = '' // Set the message text to an empty string
...
m.reply(`🚫 You are banned and cannot use the commands of this bot!\n\n${user.bannedReason ? `\n📩 *Reason:* 
            ...
            continue
             ...
             m.isCommand = true
                ...
                if (!isPrems && plugin.estrellas && global.db.data.users[m.sender].estrellas < plugin.estrellas * 1) {
                    conn.reply(m.chat, `You have run out of *Stars* 🌟\n> Use *#gift* to get free stars 💫`, m, fake)
                    continue
                }
                let extra = {
                    match,
...
botAdmin: '《★》I need to be an admin to use this function.',
unreg: `《★》You are not registered. Please register to use this function.\n*/reg name.age*\n\n*Example*: */reg lazack.18*`,
